...
\subsection{Probability density function}
Given a multivariate random variable $X \sim Dir(\alpha_1, \alpha_2, ..., \alpha_K)$, the probability density function is defined as:

$$f_{X}(x_1, x_2, ..., x_K; \alpha_1, \alpha_2, ..., \alpha_K) = \frac{1}{B(\alpha_1, \alpha_2, ..., \alpha_K)} \prod_{i=1}^{K} x_i^{\alpha_i - 1}$$

$Beta(\alpha_1, \alpha_2, ..., \alpha_K)$ is the multivariate generalization of the Beta function presented in Section \ref{betad}.

Setting $Dir(\alpha) \coloneqq Dir(\alpha_1, \alpha_2, ..., \alpha_n)$, a simplified notation of the probability density function is used:
$$f_{x}(x_1, x_2, ..., x_K; \alpha) = \frac{1}{B(\alpha)} \prod_{i=1}^{K} x_i^{\alpha - 1}$$
...
Each $p_i$ of the parameter $p = (p_1, p_2, ..., p_K)$ represents the probability of observing $i$ by sampling from the distribution.
