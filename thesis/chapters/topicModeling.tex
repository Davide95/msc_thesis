...
The generation of a document with $N$ words can be summarized as:
\begin{enumerate}
...
LDA can be seen as a probabilistic graphical model and it is represented graphically in Picture MIAOMIAO.
...
In particular, given a family of simpler distributions over the hidden structure,
we try to find the one which is the closest to the posterior distribution.
...
In our scenario, we use the Kullback–Leibler divergence as a measure of closeness.
