...
\framebox[\textwidth]{NB: this section is still a draft. Do not look at it, please. Seriously.}
...
As the Dirichlet distribution, it is a distribution over distributions.

Formally, it is defined as a two parameter distribution $\mathit{DP}(\alpha, G)$,
where $\alpha$ is the concentration parameter and G is the base distribution.

To get an intuition on how it works, it can be viewed as a black-box
which takes as input $G$ and produces a distribution $G'$
whose similarity with $G$ is governed by $\alpha$: as $\alpha \to \infty$, $G' = G$.

The procedure of generating $G$ can be described through the stick-breaking process.
