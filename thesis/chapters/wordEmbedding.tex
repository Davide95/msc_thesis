...
After a learning phase, both of them will be able to map each term $x_i \in \{0, 1\}^K$ to a point $y_i \in \mathbb{R}^V$, with $V \ll K$.
Points in the new space have a notion of similarity:
...
Suppose to have a phrase \say{A cat is the best animal.} and n-grams of size $3$.
...
This is done throught a shared matrix $M_{K \times V}$ which represents a fully connected layer.
...
Since every $x_i$ has zero values everywhere except a \say{1} in position $i$ of the vector, the result of the multiplication $M x_i$ is the row $i$ of the matrix M.
...
Finally, the last layer tries to predict all remaining words of a block in their position.
