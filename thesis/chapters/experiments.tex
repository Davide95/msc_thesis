...
since shuffling the data do not produce probability distributions. 

To overcame this issue, we built a scraper which samples pages of \url{wikipedia.it} with replacment,
using the URL \url{https://it.wikipedia.org/wiki/Speciale:PaginaCasuale}.
The basic idea is that a relatively small number of random pages does unlikely share topics and 
the resulting Hellinger distance between them can be considered noise. 
Furthermore, we have to collect enought documents to have a meaningful HDP model.
...
Since the number of Wikipedia webpages is greater than one milion\footnote{Source: \url{https://it.wikipedia.org/w/index.php?title=Speciale:Statistiche\&action=raw}},
...
This procedure was repeated 10 times with a p-value of 0.01. Results can be consulted from Figure \ref{fig:threshold-box}.
The averaged results was used to produce the two binarized network.
...
\begin{figure}[ht]
    ...
    \caption{Boxplot of the computed threshold for the filtered version (left) and the unfiltered one (right).}
