...
The final result is a set of tuples \texttt{(url, connected\_to, content)}, where \texttt{url} is the URL of a particular page, \texttt{connected\_to} is its set of links and \texttt{content} is the content of the \say{<main>} tag.
...
Note that we consider only what it is inside of the \say{<main>} tag, in order to store only the main content for each page.
