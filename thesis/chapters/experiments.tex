...
We then compared the structure of the website built through hyperlinks and the inferred network. 
In this scenario each link tag is considered an undirected and unweighted edge 
between the URL of the webpage in which it is found and the URL of its \say{href} tag.
...
\end{itemize}
where the clustering coefficient of a node is the fraction of possible triangles through that node 
and the average clustering coefficient is just an average between all node of a graph.
The histograms of the two proprieties expressed can be seen in Figure \ref{fig:exploration}. 

Thanks to this analysis we can both conclude that the inferred networks and the website structure 
are graphs with different characteristics. 
In particular, the structure is more sparse than the inferred networks and it is less clustered.
...
\begin{figure}[ht]
    ...
    and the clustering coefficient (second column).
    The first row is the resulted computations on the structure of the website, while
